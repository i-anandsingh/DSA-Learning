...
bool knight_tour(int i, int j, int n, vector<vector<int>> &board, int step){
	if(step == n*n){
		return true;
	}

	for(int d = 0; d < 8; d++){
		if(isSafe(i+x[d], j+y[d], n, board)){
			board[i+x[d]][j+y[d]] = step;
			if(knight_tour(i+x[d], j+y[d], n, board, step+1))
				return true;
			board[i+x[d]][j+y[d]] = -1;
		}
	}
